...
NAMESPACE_BEGIN(ITS)
    namespace mc::detail {
        ...
        __device__ FORCE_INLINE double
        computeSDF(const uint numNodeVerts, const thrust::pair<Eigen::Vector3d, uint32_t> *d_nodeVertexArray,
        ...
        __global__ void prepareVoxelCornerKernel(const uint nVoxels, const uint *d_voxelOffset,
                                                 const double3 *d_origin, const double3 *d_voxelSize,
                                                 const uint3 *d_res, Vector3d *d_voxelCornerData) {
            ...
            if (tid < nVoxels) {
                ...
                d_voxelCornerData[tid * 8 + 1] = voxelPos + Vector3d(0, voxelSize.y, 0);
                d_voxelCornerData[tid * 8 + 2] = voxelPos + Vector3d(voxelSize.x, voxelSize.y, 0);
                d_voxelCornerData[tid * 8 + 3] = voxelPos + Vector3d(voxelSize.x, 0, 0);
                d_voxelCornerData[tid * 8 + 4] = voxelPos + Vector3d(0, 0, voxelSize.z);
                d_voxelCornerData[tid * 8 + 5] = voxelPos + Vector3d(0, voxelSize.y, voxelSize.z);
                d_voxelCornerData[tid * 8 + 6] = voxelPos + Vector3d(voxelSize.x, voxelSize.y, voxelSize.z);
                d_voxelCornerData[tid * 8 + 7] = voxelPos + Vector3d(voxelSize.x, 0, voxelSize.z);
            }
        }
         ...
         */
        __global__ void determineVoxelKernel(const uint nVoxels,
...
#pragma unroll
         ...
         */
        __global__ void
        determineVoxelKernel_2(const uint nVoxels, const uint numNodeVerts, const double *d_isoVal,
                               const double3 *d_origin, const double3 *d_voxelSize, const uint3 *d_res,
                               const thrust::pair<Eigen::Vector3d, uint32_t> *d_nodeVertexArray,
                               const svo::SVONode *d_svoNodeArray, const double *d_lambda,
                               const cudaTextureObject_t nVertsTex, uint *d_nVoxelVertsArray,
...
//                printf("sdf[%d] = %lf\n", i, sdf[i]);
         ...
         */
        __global__ void compactVoxels(const uint nVoxels,
         ...
         */
        __global__ void voxelToMeshKernel(const uint nValidVoxels, const uint maxVerts, const double *d_isoVal,
    ...
    namespace mc {
        ...
        thrust::pair<Vector3d, uint32_t> *d_nodeVertexArray;
    ...
    namespace mc {
        ...
        void d_thrustExclusiveScan(const uint &nElems, uint *input,
                                   uint *output) {
        ...
        void launch_computSDFKernel(const uint &nVoxels,
                                    ...
                                    const VectorXd &lambda, const std::vector<Vector3d> &nodeWidthArray,
            ...
            CUDA_CHECK(cudaMemcpy(d_lambda, lambda.data(), sizeof(double) * lambda.rows(),
                                  cudaMemcpyHostToDevice)); // lambda.rows() == numNodeVerts
            ...
            for (int i = 0; i < MAX_NUM_STREAMS; ++i) {
                ...
                if (voxelOffset + voxelElems > nVoxels) {
                    lastBatch = true;
                    voxelElems = nVoxels - voxelOffset;
                }
                ...
                utils::cuda::getOccupancyMaxPotentialBlockSize(nVoxels, minGridSize, blockSize, gridSize,
                                                               detail::prepareVoxelCornerKernel);
                detail::prepareVoxelCornerKernel << < gridSize, blockSize >> >
                ...
                cuAcc::cpBSplineVal(prop, numVoxelElemCorners, numNodeVerts, d_nodeVertexArray, d_nodeWidthArray,
                                    d_lambda,
                                    d_voxelCornerData, d_voxelElemSDF, streams[i]);
        ...
        void launch_determineVoxelKernel(const uint &nVoxels,
            ...
            detail::determineVoxelKernel << < nBlocks, nThreads >> > (nVoxels, d_isoVal, nVertsTex, d_nVoxelVertsArray,
        ...
        void launch_compactVoxelsKernel(const uint &nVoxels) {
            ...
            detail::compactVoxels << < nBlocks, nThreads >> > (
        ...
        void launch_voxelToMeshKernel(const uint &maxVerts,
            ...
            detail::voxelToMeshKernel << < nBlocks, nThreads >> > (
        ...
        void marching_cubes(const std::vector<thrust::pair<Vector3d, uint32_t>> &nodeVertexArray,
                            size_t numNodes, const std::vector<Vector3d> &nodeWidthArray,
            ...
            double3 voxelSize = ::make_double3(gridWidth.x / resolution.x, gridWidth.y / resolution.y,
            ...
            if (h_voxelSDF.empty() || h_res != resolution) {
                h_res = resolution;
                launch_computSDFKernel(nVoxels, numNodes, numSVONodeVerts, lambda, nodeWidthArray, nodeVertexArray);
