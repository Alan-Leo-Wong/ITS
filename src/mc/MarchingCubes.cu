...
NAMESPACE_BEGIN(ITS)
    ...
    //__device__ inline double MCKernel::computeSDF(const uint nAllNodes, const Vector3d* d_nodeCorners,
    //	const Vector3d* d_nodeWidth, const double* d_lambda, double3 pos)
    //{
    //	double sum = 0.0;
    ...
    MCKernel::computeSDF(const uint numNodeVerts, const thrust::pair<Eigen::Vector3d, uint32_t> *d_nodeVertexArray,
...
#pragma unroll
     ...
     */
    __global__ void
    MCKernel::determineVoxelKernel_2(const uint nVoxels, const uint numNodeVerts, const double *d_isoVal,
...
//                printf("sdf[%d] = %lf\n", i, sdf[i]);
    ...
    void MC::initCommonResources(const uint &nVoxels,
    ...
    void MC::launch_computSDFKernel(const uint &nVoxels,
        ...
        CUDA_CHECK(cudaMemcpy(d_nodeVertexArray, nodeVertexArray.data(),
                              sizeof(thrust::pair<Eigen::Vector3d, uint32_t>) * numNodeVerts, cudaMemcpyHostToDevice));
        ...
        for (int i = 0; i < MAX_NUM_STREAMS; ++i) {
            ...
            cuAcc::cpBSplineVal(prop, numVoxelElemCorners, numNodeVerts, d_nodeVertexArray, d_nodeWidthArray, d_lambda,
    ...
    void MC::launch_compactVoxelsKernel(const uint &nVoxels) {
        // init resources
     ...
     */
    void MC::marching_cubes(const std::vector<thrust::pair<Vector3d, uint32_t>> &nodeVertexArray,
    ...
    void MC::marching_cubes(uint3 resolution,
                            double3 gridOrigin,
                            double3 gridWidth,
                            double isoVal,
