...
std::tuple<UINT, UINT, string, string> execArgParser(int argc, char** argv)
{
	...
	CmdLineParameter<string> _mp_1("mp1");     // path of the first model
	CmdLineParameter<string> _mp_2("mp2");     // path of the first model
	...
	const string mp_1 = _mp_1.set ? _mp_1.value : "";
	const string mp_2 = _mp_2.set ? _mp_2.value : "";

	if (mp_1.empty() || mp_2.empty())
	{
