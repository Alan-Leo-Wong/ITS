...
#pragma omp parallel
            for (const auto &tri: modelTris) {
                V3d triEdge_1 = tri.p2 - tri.p1;
                V3d triEdge_2 = tri.p3 - tri.p2;
                V3d triEdge_3 = tri.p1 - tri.p3;
                V3d triNormal = tri.normal;
...
#pragma omp for nowait
                    for (int i = x_lower - x_fineNodeEdges.begin(); i < x_numEdges; ++i) {
                        ...
                        if (isLargeDouble(e_p1.x(), tri_bbox_end.x(), 1e-9)) break; // ��ʼ�˵����bbox_end
...
#pragma omp for nowait
                    for (int i = y_lower - y_fineNodeEdges.begin(); i < y_numEdges; ++i) {
                        ...
                        if (isLargeDouble(e_p1.y(), tri_bbox_end.y(), 1e-9)) break; // ��ʼ�˵����bbox_end
...
#pragma omp for nowait
                    for (int i = z_lower - z_fineNodeEdges.begin(); i < z_numEdges; ++i) {
                        ...
                        if (isLargeDouble(e_p1.z(), tri_bbox_end.z(), 1e-9)) break; // ��ʼ�˵����bbox_end
...
#pragma omp critical
        ...
        inline void ThinShells::cpSDFOfTreeNodes() {
...

            {
                VXi I;
                MXd C, N;
                igl::signed_distance(pointsMat, m_V, m_F,
...
#pragma omp parallel for reduction(+ : sum) // forѭ���еı�����������з�������
        ...
        void ThinShells::singlePointQuery(const std::string &out_file, const V3d &point) {
            ...
            if (innerShellIsoVal < q_bSplineVal && q_bSplineVal < outerShellIsoVal)
                rgb = V3d(0.56471, 0.93333, 0.56471);
            else rgb = V3d(1, 0.27059, 0);

            string _out_file = out_file;
            if (getFileExtension(_out_file) != ".obj")
                _out_file = (string) getDirName(DELIMITER, out_file.c_str()) +
                            (string) getFileName(DELIMITER, out_file.c_str()) + (string) ".obj";
            ...
            std::transform(q_bSplineVal.begin(), q_bSplineVal.end(), std::back_inserter(rgbs),
                           [=](double val) {
                               ...
                               if (innerShellIsoVal < val && val < outerShellIsoVal)
                                   _t = V3d(0.56471, 0.93333, 0.56471);
...
// ����ר��
        ...
        vector<int> ThinShells::multiPointQuery(const vector <V3d> &points, double &time, const int &session,
...
#pragma omp simd simdlen(8)
        ...
        inline void ThinShells::prepareMoveOnSurface(int &ac_treeDepth,
            ...
            if (treeDepth < 1) {
                fprintf(stderr, "Error: SVO is empty!");
                return;
            }
            ...
            morton2Nodes.resize(ac_treeDepth); // 2�㼴��
            nodeWidth.resize(ac_treeDepth);
            nodeOrigin.resize(ac_treeDepth);

            vector <size_t> esumDepthNumNodes(ac_treeDepth);
            std::exclusive_scan(depthNumNodes.begin(), depthNumNodes.begin() + ac_treeDepth, esumDepthNumNodes.begin(),
                                0);
            esumDepthNumNodes.emplace_back(
                    (*esumDepthNumNodes.rbegin()) + depthNumNodes[ac_treeDepth - 1]); // ��������ac_treeDepth��Ľڵ���

            for (int d = 0; d < ac_treeDepth; ++d) {
                vector <size_t> d_numNodesIdx(depthNumNodes[d]);
                std::iota(d_numNodesIdx.begin(), d_numNodesIdx.end(), 0);

                std::transform(nodeArray.begin() + esumDepthNumNodes[d],
                               nodeArray.begin() + esumDepthNumNodes[d] + depthNumNodes[d],
        ...
        void ThinShells::moveOnSurface(const V3d &modelVert, const V3d &v, const size_t &max_move_cnt) {
            ...
            // �ҵ���������ӵ�ԭ�㡢����Լ��˸������b����ֵ
            auto getNodeBSplineVal = [=](const V3d &vert,
                                         bool &flag) -> std::tuple<V3d, double, std::array<double, 8>> {
                // ������ڵ�0���ǿ϶����ڵ�1��
                for (int d = 0; d < ac_treeDepth; ++d) {
            ...
            while (valid_move < max_move_cnt) {
                ...
                do {
                    ...
                    dir = Eigen::Matrix<double, 3, 1>::Zero(3, 1)
                            .unaryExpr(
                                    [&](double val) { return static_cast<double>(round((n(e) + 1e-6) * 1e6) / 1e6); });
                ...
                //if (toOuterShell.isApprox(zeroVec, 1e-9) || toInnerShell.isApprox(zeroVec, 1e-9)) // ���������ǻ����ڿ���
                //{
                //	// ���һ���µķ���
                //	dir = Eigen::Matrix<double, 3, 1>::Zero(3, 1)
                ...
                point = new_point;
                printf("#%llu: point at (%lf, %lf, %lf), bSplineVal = %lf\n", valid_move, point.x(), point.y(),
                       point.z(), point_bSplineVal);
        ...
        void ThinShells::launchParticleSystem(const int &maxIterations, const std::string &out_file) {
            ...
            particleArray = m_V;
            //#pragma omp parallel for reduction(+ : embedOmiga) // ֻ�б�����̬���ݳ�Ա�������� data - sharing �Ӿ���
...
#pragma omp parallel
            for (int i = 0; i < numPoints; ++i) {
                ...
                auto inDmPointsTraits = svo.mq_setInDomainPoints(pointMorton, modelOrigin, searchNodeWidth, 0,
                                                                 depthMorton2Nodes, depthVert2Idx);
                const int nInDmPointsTraits = inDmPointsTraits.size();

                //#pragma omp parallel for reduction(+ : gradient) // forѭ���еı�����������з�������
                for (int j = 0; j < nInDmPointsTraits; ++j) {
                    const auto &inDmPointTrait = inDmPointsTraits[j];
...
#pragma omp parallel for reduction(+ : sum) // forѭ���еı�����������з�������
        ...
        inline std::pair<VXd, MXd>
        ThinShells::getPointValGradient(const MXd &before_queryPointMat, const MXd &queryPointMat) {
            const int numPoints = queryPointMat.rows();
...
#pragma omp parallel
        ...
        inline MXd
        ThinShells::getProjectPoint(const MXd &before_queryPointMat, const MXd &queryPointMat, const int &iter) {
            //const auto& nodeArray = svo.svoNodeArray;
        ...
        void ThinShells::lbfgs_optimization(const int &maxIterations, const std::string &out_file) {
            ...
            auto updateNeighbor = [&](const MXd &queryPointMat) {
                ...
                neighPointList.resize(numParticles);

                for (auto &innerVec: fineNodeParticle) innerVec.clear();
                fineNodeParticle.clear();
                fineNodeParticle.resize(svo.numTreeNodes);
                ...
                for (int i = 0; i < numParticles; ++i) {
                    ...
                    uint32_t mortonCode = morton::mortonEncode_LUT((uint16_t) dis.x(), (uint16_t) dis.y(),
                                                                   (uint16_t) dis.z());
                    int depth = 0;
                    while (depth < treeDepth &&
                           depthMorton2Nodes[depth].find(mortonCode) == depthMorton2Nodes[depth].end()) {
                        ++depth;
                        mortonCode /= 8;
                    }
                    if (depth == treeDepth) {
            ...
            auto optimize_fun = [&](const Eigen::VectorXd &before_particle, Eigen::VectorXd &grad) {
                ...
                for (size_t i = 0; i < numParticles; ++i)
                    before_particleMat.row(i) = Eigen::Vector3d(before_particle(i * 3), before_particle(i * 3 + 1),
                ...
                for (int i = 0; i < numParticles; ++i) {
                    ...
                    for (int j = 0; j < numSearch; ++j) {
                        ...
                        const double &ij_energy = std::exp(
                                -((i_particle - j_neiParticle).squaredNorm()) / (4 * theta * theta));
                        systemEnergy += ij_energy;
                        i_force += ((i_particle - j_neiParticle) / (2 * theta * theta)) * ij_energy;
                    }
            ...
            for (int iter = 1; iter <= maxIterations; ++iter) {
                if (iter == 1) {
                    proj_particleMat = particleArray;
                    normal = m_VN;
                }
                ...
                MatrixXd particleMat(numParticles, 3);
                //#pragma omp parallel for
                for (int i = 0; i < numParticles; ++i)
                    particleMat.row(i) = Eigen::Vector3d(particle_x(i * 3), particle_x(i * 3 + 1),
                                                         particle_x(i * 3 + 2));

                const string tempResFile = concatFilePath((string) VIS_DIR, modelName, uniformDir,
                                                          std::to_string(treeDepth), (string) "temp_particle.xyz");
                ...
                const string tempProjFile = concatFilePath((string) VIS_DIR, modelName, uniformDir,
                                                           std::to_string(treeDepth), (string) "proj_particle.xyz");
                std::ofstream out2(tempProjFile);
                gvis::writePointCloud_xyz(proj_particleMat, out2);
                //system("pause");
            }
            //proj_particleMat = getProjectPoint(proj_particleMat, 10); // ����ͶӰ
            VectorXd sqrD;
            VectorXi I;
            MatrixXd C;
            ...
            if (!out_file.empty()) {
                ...
                if (!out) {
                    fprintf(stderr, "[I/O] Error: File %s could not be opened!", out_file.c_str());
                    return;
                }
